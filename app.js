...
// -----------------------
// Main interactive logic for the page:
//
// - Loads Netflix/OMDb/TMDB data from netflix_omdb_master.csv
// - Builds the scatterplot "Hit Matrix" of genres
// - Handles crossfades into horror-only views, posters, and the world map
// - Manages shared tooltip, fine-print note, and scroll-driven scene changes
...
function renderPosterWall(data) {
  ...
  posterWallGrid.selectAll("img.poster-cluster").remove();
  ...
  async function showPosterWall() {
    ...
    // Fade chart out first, then reveal poster wall
    crossfadeOut([gChart, gAxis, gLegend], 180, d3.easeCubicOut, () => {
      posterWall
        .style("display", "block")
        .style("opacity", 0)
  ...
  async function ensureClusterPosterRows() {
    if (clusterPosterRows) return clusterPosterRows;

    const raw = await d3.csv("./poster_wall_clusters_hero.csv");

    clusterPosterRows = raw.map(d => ({
      poster:  d.poster,
      title:   d.title || "",
      country: d.country || "Unknown",
      cluster: d.cluster || ""
    ...
    return clusterPosterRows;
  }

  let clusterAllData = null;
  let clusterCurrentGroup = -1;
  let clusterScrollListener = null;

  function renderPosterWallClusters(data) {
  clusterAllData = data;
  clusterCurrentGroup = -1;
  ...
  const cols = 4;
  const rows = 2;
  const gridHeight = (posterHeight * rows) + (gap * (rows - 1));
  
  posterWallGrid.selectAll("img").remove();

  posterWallGrid.classed("cluster-grid", true);
  posterWallGrid
    .style("grid-template-columns", `repeat(${cols}, ${posterWidth}px)`)
    .style("margin", "0")
    ...
    const totalGroups = Math.ceil(data.length / 8);
    
    // Create 8 image slots (fixed 4x2 grid)
    for (let i = 0; i < 8; i++) {
      posterWallGrid.append("img")
        .attr("class", "poster-cluster")
        .attr("alt", "")
        .attr("loading", "lazy")
        .attr("decoding", "async")
        .attr("src", "")
        ...
        .on("mouseover", function() {
          ...
          if (!item) return;
          const countries = item.country || "Unknown";
          const label = countries.includes(",") ? "Production countries:" : "Production country:";
          const clusterLabel = item.cluster ? `<br/><span style="font-size:12px;color:#666;">Cluster ${item.cluster}</span>` : "";
          tooltip
            .style("opacity", 1)
            .html(
              `<strong>${item.title || "Untitled"}</strong><br/>` +
              `<span style="font-size:12px;color:#666;">${label}</span> ` +
              `${countries}${clusterLabel}`
            );
        })
        .on("mousemove", event => {
          tooltip
    ...
    if (clusterScrollListener) {
      window.removeEventListener("scroll", clusterScrollListener, { passive: true });
    }

    // Setup scroll listener to swap groups
    clusterScrollListener = () => {
      const posterCard = document.querySelector('.step[data-scene="posters_clusters"]');
      if (!posterCard || !clusterAllData) return;
      ...
      // Determine which group to show (0, 1, 2, or 3)
      const newGroup = Math.floor(scrollProgress * totalGroups);
      const groupClamped = Math.min(newGroup, totalGroups - 1);
      
      if (groupClamped !== clusterCurrentGroup) {
        clusterCurrentGroup = groupClamped;
        updateClusterGroup(groupClamped);
      }
    };

    window.addEventListener("scroll", clusterScrollListener, { passive: true });
    
    // Show first group immediately
    updateClusterGroup(0);
  }
  
  function updateClusterGroup(groupIndex) {
    const startIdx = groupIndex * 8;
    posterWallGrid.selectAll("img.poster-cluster").each(function(d, i) {
      const dataIdx = startIdx + i;
      const item = clusterAllData[dataIdx] || null;
  ...
  async function showPosterWallClusters() {
    setTitleForPosters();    
    positionPosterWallToSVG();

    const data = await ensureClusterPosterRows();
    if (!data.length) return;

    renderPosterWallClusters(data);
  ...
  function go(scene) {
    // Restore the graphic card for all non-closing scenes
    if (scene !== "noop") {
      d3.select(".graphic")
        .style("opacity", null)
    ...
    if (scene === "baseline") {
    ...
    else if (scene === "posters_clusters") {
      hideWorldMap();
      showMethodologyBtn();
      hideFineNote();
      ...
      // closing — hide everything, let the text card stand alone
      hideWorldMap();
      hidePosterWall();
      hideMethodologyBtn();
      hideFineNote();
